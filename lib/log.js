...
function commitAndPush(message) {
  ...
  try {
    execSync(
      `git -C "${REPO_DIR}" remote set-url origin https://${token}@github.com/waittinghsu/twice-prize-tracker.git`,
      { stdio: 'pipe' }
    )
    ...
    // Push log.md to logs branch without triggering Zeabur redeploy
    execSync(`git -C "${REPO_DIR}" stash`, { stdio: 'pipe' })
    execSync(`git -C "${REPO_DIR}" fetch origin main logs`, { stdio: 'pipe' })
    ...
    execSync(`git -C "${REPO_DIR}" stash pop`, { stdio: 'pipe' })
